...
--
-- "--language auto" is required: the config default is Swedish, so without
-- an explicit override this preset would silently force sv.
local PRESET_ARGS = "--language auto --max-words 0 --max-chars 42 --max-lines 1"

print("[LAB37 Auto] Starting...")
...
if lf then lf:close() else
    print("[LAB37 Auto] ERROR: fusionscript.so not found at: " .. RESOLVE_LIB)
    print("[LAB37 Auto] Is Resolve installed in a nonstandard location?")
    return
end
...
if not f then
    print("[LAB37 Auto] ERROR: pointer file not found")
    return
end
...
print("[LAB37 Auto] Launching subprocess.")
os.execute(cmd)
